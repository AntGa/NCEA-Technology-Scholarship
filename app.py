...
def signup():
    ...
    if request.method == "POST":
        ...
        if userpassword == userconfirmpassword:
            #hash password using hashlib.md5
            hash_obj = hashlib.md5(userpassword.encode())
            hashed_password = hash_obj.hexdigest()

            #insert hashed password and details into database
...
def dashboard():
    ...
    cur6.execute("SELECT locationTotal FROM locations WHERE username = %s", [username])
    locationtotals = cur6.fetchone()
    if locationtotals is None:
        locationtotal = "none"
    
    for row in locationtotals:
        locationtotal = locationtotals[0]
        print(locationtotal)


    

    return render_template("main-menu.html", salesrows=salesrows, productrows=productrows, totalsales=totalsales, totalsalesprice=totalsalesprice, productrows2=productrows2, locationtotal=locationtotal)
...
def update_view_item():
    if request.method == 'POST':
        ...
        #elif print the barcode on a seperate page
...
def create_sales():
    ...
    if request.method == "POST":
        ...
        cur4 = mysql.connection.cursor()
        cur4.execute("UPDATE locations SET locationTotal=%s WHERE username =%s",(saleLocation, username))
        mysql.connection.commit()
...
def update_sale():
    ...
    if request.method == "POST":
        if request.form.get("deleteButton"):
            ...
            cur5 = mysql.connection.cursor()
            cur5.execute("UPDATE locations SET locationTotal=%s WHERE username =%s",(saleLocation, username))
            mysql.connection.commit()
            #update the sale
